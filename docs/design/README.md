# Design docs

These docs cover the design principles and best practices for agentic software development that have been influential in the design of the agent skills in this collection.

- [**What is agentic software development?**](./agentic-definition.md)
- [**Agentic versus automated**](./agentic-vs-automated.md)
- [**Objectives of agentic workflow design**](./objectives.md)
- [**Guides and sensors**](./guides-and-sensors.md)
- [**Calibration**](./calibration.md)
- [**Deterministic sensors**](./deterministic-sensors.md)
- [**Inferential sensors**](./inferential-sensors.md)
- [**Composable pipelines**](./composable-pipelines.md)
- [**Iterative and incremental**](./iterative-and-incremental.md)
- [**Single responsibility**](./single-responsibility.md)
- [**Rules versus knowledge**](./rules-versus-knowledge.md)
- [**Loose coupling**](./loose-coupling.md)
- [**Interface definitions**](./interface-definitions.md)
- [**Persistence**](./persistence.md)
- [**Version control as the substrate**](./version-control-substrate.md)
- [**Isolated environments**](./isolated-environments.md)
- [**Interactive versus non-interactive**](./interactivity.md)
- [**Human-in-the-loop**](./human-in-the-loop.md)
