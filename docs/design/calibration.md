...
For the same reason, agent skills (or other guides) work best when they have a certain rigidity — clear, unambiguous, prescriptive step-by-step instructions (ie. reusable procedures) for agents to follow, and success criteria that can be verified with a deterministic test. For example, rather than giving agents a menu of equally-weighted options,prefer to provide a default with named alternatives for specific, clearly-articulated use cases. Match the specificity of an instruction to the fragility of the task at hand. Give the agent some latitude where the task tolerates variation, but explain *why* different approaches will be valid in different scenarios.
