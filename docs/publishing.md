...
- Each skill must be discoverable by the `skills` CLI. It will look for a root-level `SKILL.md`, or a `skills/` directory following the [Agent Skills](https://agentskills.io/) conventions.
...
- `notGrouped`: Where skills not assigned to any group appear. The value `"bottom"` pushes them below the defined groups.

- `groupings`: Ordered list of display groups, each with a `title`, `description`, and array of skill `name` values matching the `name` field in each `SKILL.md` front-matter.
