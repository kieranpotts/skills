---
name: branch
description: Git branching conventions, including the trunk model (`dev` → `test` → `ready`), short-lived `temp/*` branches, and long-lived `epic/*` branches, plus naming rules and validation regex. Use when creating a new branch, naming a feature or fix branch, or validating branch names before push, or when the user says "what should I call this branch?", "create a branch for this work", or "is this branch name valid?".
