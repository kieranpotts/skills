...
1.  **Run the check and read its output literally.**
    ...
    import, a type mismatch, an undefined symbol) — that message is the spec for
    the fix.

2.  **If auditing rather than responding to a specific failure, run every
    available check.**

    When asked to find what's broken rather than fix a named failure, run the
    project's build, compiler, linter, type-checker, and any other static gate,
    in turn. Compile a list of distinct issues before fixing any — fixing one
    can sometimes resolve or mask another.
...
3.  **Prefer automated fixes where they exist.**
    ...
    warnings have an automated migration codemod. Run these first, then re-check
    — automated fixes rarely resolve everything.

4.  **Fix remaining issues at their reported location.**

    Make the minimal change that resolves the specific error — not a broader
...
5.  **If a check is wrong for this case, suppress it explicitly — never
    ...
    Some violations are false positives for the specific context. Suppress with
    an inline directive (`// eslint-disable-next-line <rule> -- <reason>`, `#
    type: ignore[<code>] -- <reason>`) that names the rule and states why it
    doesn't apply here. An unexplained suppression is worse than the violation —
    the next reader can't tell if it was a judgment call or laziness.
