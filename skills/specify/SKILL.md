...
2.  **Validate the PRD for completeness.**
    ...
    If any of these is missing or ambiguous, reject the PRD, and
    output a specific, itemized list of what is absent or unclear. Purely
    mechanical gaps (a `Feature` title derived from the goal,
    scenario ordering, tidy phrasing) can be normalized without rejecting; the bar for rejection is
...
4.  **Read the SRS repository's `AGENTS.md` to learn its workflow.**
    ...
    label rules, and follow whatever it prescribes.
...
5.  **Scaffold the proposal — follow `draft-spec`.**
    ...
    request, and the discussion thread. Derive the change description,
    slug, and change type from the PRD's outcome rather than prompting for them.
    Preserve the originating PRD as the origin artifact if the
...
7.  **Mark the proposal ready — follow `propose-spec`.**
    ...
    carry out its checks. Verify completeness, then take the pull request
...
8.  **Report the outcome and the required approval.**
    ...
    before downstream work begins. State explicitly that the next SDLC
    phase — design — does not start until this specification is approved
    (`ACCEPTED`). Link the pull request and its discussion thread.
